...
**概念定义**：语义搜索系统是一种基于向量相似度的信息检索架构——先将文档通过 Embedding 转为向量存入数据库，再将用户查询转为同一向量空间中的向量，通过计算向量距离找到最相关的内容。

**核心定位**：传统的关键词搜索依赖精确匹配，无法理解同义词或语义关联。语义搜索系统利用向量 Embedding 将"搜索"从关键词匹配升级为语义理解，让用户用自然语言就能找到想要的信息。
...
1. **分块策略选择不当导致检索质量差**：chunk_size 太小（< 200）导致语义碎片化，太大（> 2000）导致上下文稀释。建议根据文档类型选择合适的 chunk_size，一般中文文档 500-1000 较优。

2. **Embedding 模型在开发和生产环境不一致**：开发时用 OpenAI Embedding 建索引，部署后改用本地模型，导致向量空间不匹配、检索准确率骤降。建议在项目初期就确定 Embedding 模型并固定下来，切换模型时必须重建索引。

3. **未处理查询为空或无效输入**：用户输入空字符串或纯标点符号时，Embedding 模型仍会生成向量但检索无意义。建议在搜索接口层对输入做长度和有效性校验。

4. **重排序逻辑未考虑性能开销**：对 Top-100 结果做 LLM 重排序，每次搜索耗时数秒，严重影响用户体验。建议将重排序限制在 Top-10 以内，或者使用轻量级 Cross-Encoder 模型替代 LLM。

5. **API 服务未做并发控制导致连接耗尽**：FastAPI 多线程同时访问 Chroma/Milvus 时可能产生连接冲突。建议使用连接池或每次请求创建独立客户端实例。

---

## 课后练习

1. 从你的电脑上选择 20 个文档（.txt、.md、.pdf 均可），构建一个完整的语义搜索系统，包括文档加载、分块、向量化、检索和展示。

2. 在搜索系统中加入结果重排序模块，对比有无重排序的前 5 条搜索结果质量差异。

3. 使用 FastAPI 将语义搜索系统包装为 RESTful API 服务，支持 POST /search 和 POST /documents 两个接口。

4. 设计一个 A/B 测试方案：对比纯关键词搜索和语义搜索在同一个数据集上的搜索效果，从召回率和用户满意度两个维度评估。
